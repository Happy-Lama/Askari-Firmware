...
void update_alarm_state(){
  ...
  //saves the current alarm state in eeprom memory
...
void update_passcode_eeprom(String passcode){

  //saves the current passcode in eeprom memory

  for(uint8_t i = 0; i < 4; i++){
    EEPROM.update(PASSCODE_IDX + i, passcode[i]);
    delay(100);
  }
}
